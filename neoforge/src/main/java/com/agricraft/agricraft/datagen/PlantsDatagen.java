...
public class PlantsDatagen {
	...
	public static void registerPlants(BootstrapContext<AgriPlant> context) {
		...
		agricraft(context, "nethereed", new AgriPlant.Builder().stages16().chances(0.15, 0.015, 0.025).products(AgriProduct.builder().item("agricraft:netherite_sliver").count(1, 1, 0.25).build()).harvest(0).requirement(AgriRequirement.builder().humidity(DRY, EQUAL, 0.2).acidity(SLIGHTLY_ALKALINE, EQUAL, 0.2).nutrients(LOW, EQUAL, 0.2).dimensions(Level.NETHER.location()).seasons(SUMMER).blocks(AgriBlockCondition.builder().tag("agricraft:ores/netherite_scrap").build()).fluid(AgriFluidCondition.builder().fluid("minecraft:lava").build()).build()).modifiers(new AgriPlantModifierInfo(MinecraftPlantModifiers.ThornsPlantModifier.ID), new AgriPlantModifierInfo(MinecraftPlantModifiers.BushyPlantModifier.ID), new AgriPlantModifierInfo(MinecraftPlantModifiers.BurnPlantModifier.ID)).fireproof(true).build());
